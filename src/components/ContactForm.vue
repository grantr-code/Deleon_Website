<template>
  <form @submit.prevent="onSubmit" class="mx-auto max-w-full">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div>
        <label for="cf-name" class="block text-sm text-foreground/80 mb-1">Full name</label>
        <input id="cf-name" v-model.trim="name" type="text" required :class="inputClass" placeholder="Jane Doe" />
      </div>
      <div>
        <label for="cf-email" class="block text-sm text-foreground/80 mb-1">Email</label>
        <input id="cf-email" v-model.trim="email" type="email" required :class="inputClass" placeholder="you@company.com" />
      </div>
      <div>
        <label for="cf-company" class="block text-sm text-foreground/80 mb-1">Company/Team</label>
        <input id="cf-company" v-model.trim="company" type="text" :class="inputClass" placeholder="Acme Inc." />
      </div>
      <div>
        <label for="cf-phone" class="block text-sm text-foreground/80 mb-1">Phone (optional)</label>
        <input id="cf-phone" v-model.trim="phone" type="tel" :class="inputClass" placeholder="+1 (555) 123‑4567" />
      </div>
      <div class="sm:col-span-2">
        <label for="cf-reason" class="block text-sm text-foreground/80 mb-1">Reason</label>
      ...
      <div class="sm:col-span-2">
        <label for="cf-message" class="block text-sm text-foreground/80 mb-1">Message</label>
      ...
      <div class="sm:col-span-2 flex items-center gap-2 mt-1">
        <input id="cf-updates" v-model="updates" type="checkbox" class="h-4 w-4 rounded border-border bg-muted/10" />
        <label for="cf-updates" class="text-sm text-foreground/80">Also send me product updates</label>
      </div>
      <div class="sm:col-span-2 mt-2 flex items-center gap-3">
        <button type="submit" :disabled="submitting || success" class="rounded-full bg-accent text-black font-semibold px-5 py-3 whitespace-nowrap hover:brightness-110 disabled:opacity-60 disabled:cursor-not-allowed">
...
const inputBase = 'w-full rounded-lg px-4 py-2.5 text-base bg-muted/10 border border-border text-foreground placeholder-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent/60';
