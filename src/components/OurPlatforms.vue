  ...
  <section>
    ...
    >
      ...
      <div v-if="!hideHeader" :class="['relative', mode === 'full' ? 'mb-8' : (mode === 'slideshow' ? 'mb-4 md:mb-5' : 'mb-3 md:mb-4')]">
        ...
        <h2 class="mt-3 md:mt-4 text-[clamp(28px,4.8vw,58px)] leading-tight max-w-[1100px] text-foreground mx-auto text-center">
          <template v-if="headlineParts">
            <span>{{ headlineParts.before }}</span>
            <span class="text-muted-foreground">{{ headlineParts.match }}</span>
            <span>{{ headlineParts.after }}</span>
          </template>
          <template v-else>{{ headline }}</template>
        </h2>
      </div>
      <div v-if="!hideHeader && mode !== 'full'" class="border-t border-border my-3 md:my-4"></div>

      <!-- Mobile: slideshow above the list -->
      <div v-if="mode === 'mobile'" class="grid grid-cols-12 gap-6">
        <!-- Slideshow block on top -->
        <div class="col-span-12">
          <div class="relative h-[320px] sm:h-[380px] rounded-[10px] border border-border overflow-hidden bg-card">
            <div class="absolute inset-0">
              <transition name="slide-fade">
                <div :key="idx" class="absolute inset-0">
                  <component :is="canvasFor(items[idx])" v-if="canvasFor(items[idx])" />
                  <div class="absolute inset-0 bg-gradient-to-b from-white/10 dark:from-black/10 to-white/60 dark:to-black/60"></div>
                  <div class="relative z-10 h-full flex flex-col justify-center gap-4 p-5 sm:p-6 text-foreground">
                    <div>
                      <h3 class="text-[clamp(20px,5.0vw,30px)] leading-[1.1] mb-2">{{ items[idx].title }}</h3>
                      <p class="text-muted-foreground text-sm max-w-[60ch]">{{ items[idx].long }}</p>
                    </div>
                  </div>
                </div>
              </transition>
            ...
            <div class="absolute bottom-3 right-3 flex gap-2 z-20">
              <button @click="prev" class="p-2 rounded-md bg-muted/10 border border-border text-foreground hover:bg-muted/20" aria-label="Previous">‹</button>
              <button @click="next" class="p-2 rounded-md bg-muted/10 border border-border text-foreground hover:bg-muted/20" aria-label="Next">›</button>
            </div>
          </div>
        </div>
        ...
        <!-- List below -->
        <div class="col-span-12">
          <ul role="tablist" aria-label="Platforms" class="divide-y divide-border rounded-[10px] overflow-hidden bg-transparent">
                ...
                <div class="flex items-baseline justify-between gap-4">
                  <div>
                    <TypeOnView :text="p.name" :speed="24" tag="div"
                      wrapper-class="text-[clamp(26px,5.6vw,48px)] leading-none text-foreground" />
                    <p class="text-muted-foreground mt-2 max-w-[42ch]">{{ p.description }}</p>
                  </div>
                  <div class="text-muted-foreground text-sm">/{{ formatIndex(i) }}</div>
                </div>
              </div>
            </li>
          </ul>
      ...
      <div v-if="mode === 'slideshow'" class="grid grid-cols-12 gap-6">
          ...
          <ul role="tablist" aria-label="Platforms" class="divide-y divide-border rounded-[10px] overflow-hidden bg-transparent">
            <li v-for="(p, i) in items" :key="i" role="presentation">
              <button
                ...
                :tabindex="i === idx ? 0 : -1"
                class="w-full text-left px-4 md:px-5 py-6 md:py-8 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/60"
                @click="set(i)"
                @mouseenter="set(i)"
              >
                <div class="flex items-baseline justify-between gap-4">
                  <div>
                    <TypeOnView :text="p.name" :speed="24" tag="div"
                      wrapper-class="text-[clamp(26px,5.6vw,48px)] leading-none text-foreground" />
                    <p class="text-muted-foreground text-sm md:text-base mt-2 max-w-[60ch]">{{ p.long || p.description }}</p>
                  </div>
                  <div class="text-muted-foreground text-sm">/{{ formatIndex(i) }}</div>
                </div>
              </button>
        ...
        <div class="col-span-12 md:col-span-6">
          <div class="sticky top-10">
            <div class="relative h-[420px] sm:h-[480px] md:h-[560px] rounded-[10px] border border-border overflow-hidden bg-card">
              <!-- Slides -->
              <div class="absolute inset-0">
                <transition name="slide-fade">
                  <div :key="idx" class="absolute inset-0">
                    <component :is="canvasFor(items[idx])" v-if="canvasFor(items[idx])" />
                    <div class="absolute inset-0 bg-gradient-to-b from-white/10 dark:from-black/10 to-white/60 dark:to-black/60"></div>
                    <div class="relative z-10 h-full flex flex-col justify-center gap-4 md:gap-6 p-5 sm:p-6 md:p-8 text-foreground">
                      <div>
                        <h3 class="text-[clamp(24px,4.2vw,46px)] leading-[1.05] mb-3">{{ items[idx].title }}</h3>
                        <p class="text-muted-foreground text-sm md:text-base max-w-[60ch]">{{ items[idx].long }}</p>
                      </div>
                    </div>
                  </div>
                </transition>
    ...
    <div v-if="mode === 'full'" class="border-t border-border my-0"></div>
    ...
    <div v-if="mode === 'full'" class="mt-4 md:mt-5">
      <div class="space-y-10">
          <div
            v-for="(p, i) in items"
            :key="'wide-' + i"
          class="group relative overflow-hidden border-y border-border bg-muted/5 hover:bg-muted/10 transition-colors"
          :style="fullRowStyle"
        ...
        >
          ...
          <div
            class="absolute inset-y-0 rounded-none overflow-hidden border-x border-y-0 border-border dark:shadow-[0_10px_30px_-10px_rgba(0,0,0,0.6)]"
          ...
          >
            ...
            <div class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
              ...
              <div v-else class="absolute inset-0 bg-gradient-to-br from-muted/10 dark:from-white/5 to-muted/5 dark:to-white/[.02]"></div>
              <div class="absolute inset-0 bg-white/25 dark:bg-black/25"></div>
            </div>
            ...
            <div class="absolute inset-0 z-10 flex flex-col justify-center p-5 sm:p-6 xl:p-8 transition-opacity duration-300 pointer-events-none group-hover:opacity-0">
              <p class="text-muted-foreground max-w-none text-[clamp(12px,1.0vw,16px)] leading-relaxed">{{ p.description }}</p>
              <div class="text-muted-foreground text-sm mt-2">/{{ formatIndex(i) }}</div>
            </div>
            ...
            <div class="absolute inset-0 z-10 flex flex-col justify-center p-5 sm:p-6 xl:p-8 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
              <p class="text-muted-foreground max-w-none text-[clamp(12px,0.95vw,16px)] leading-relaxed">{{ p.long || p.description }}</p>
              <div class="text-muted-foreground text-sm mt-2 opacity-0">/{{ formatIndex(i) }}</div>
            </div>
          </div>
          ...
          <div class="relative z-10 grid grid-cols-12 gap-6 items-center py-8 md:py-12 px-0">
            <div class="col-span-12 md:col-span-12 md:text-right pr-0">
              <TypeOnView
                ...
                :speed="24"
                tag="div"
                wrapper-class="inline-block leading-[0.88] text-foreground tracking-[-0.02em] text-[min(13vw,132px)] pr-0"
              />
            </div>
          </div>
        </div>
